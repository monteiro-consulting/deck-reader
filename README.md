...
- **Series C and every later round** (series D, growth rounds): the same engine, another
...
picks the grid (`scripts/grids/preseed.json`, `seed.json`, `series_a.json`, `series_b.json`, `series_c.json`). The detected
...
seed, series A, series B and series C, when documents are missing, `path/to/deck.founder-email.md`. Add
...
### Series A, series B and series C, between the profile and the grid

| Step | What happens | Who |
|---|---|---|
| 3A. Documents | Each annex sorted into one of the required documents of the stage and model with a verbatim quote and the months or items covered; the quote is checked by code. First gate: the list is the one of the stage grid (six documents for SaaS at series A, ten at series B, eleven at series C) adjusted by the model block, never by the deck; a document missing, or covering fewer months or items than required, stops the reading with the request email. No coverage threshold | `annex_text.py`, `annex-classifier`, `documents_gate.py`, `founder_email.py` |
| 4A. Claims and proof | As at seed, with the series A, series B or series C claim types. At series C a plan vs actual claim is looked up twice, the plan in the board pack and the actual in the P&L. A figure the deck attributes to the audited accounts is looked up there, not in the P&L. Claims the documents do not cover are listed, never a stop. No leftovers email: the document list replaces it | `claim-extractor`, `check_claims.py`, `annex-matcher`, `verify_matches.py` |
| 5S, 6S | As at seed, with public reviews, open job posts, the press of previous rounds and the LinkedIn profile of each announced key hire added to the web scope; at series B also every executive on LinkedIn, the headcount trend and departures, employee reviews, job posts by country, registries for announced subsidiaries, the press of every round; at series C also the accounts filed at the company registry against the audited ones, the pricing page history on the Wayback Machine, the review trend over 24 months, litigation and security incidents made public, the rounds of competitors since the series B, listed comparables | `web-verifier`, `verify_web.py`, `contradiction-reviewer`, `documents_gate.py` |
| 7. Report | Benchmarks for the model and the stage shown next to each figure, with source and date. Displayed, never scored. At series C, the plan against actual table, quarter by quarter, laid out by code | `report.py`, `scripts/grids/benchmarks/` |
...
| `documents` | the required document list of the first gate (`annex_gate.required_documents`, series A, series B and series C) adjusted: `remove` by document id, `add` with entries of the grid's shape (`id`, `name`, `requirement`, `min_months`, optional `min_count`). Only where the stage grid has a list; an error otherwise |
...
skills/deck-reader/grids/series_c.md   the series C grid (and every later round), readable, with the eleven-document list
...
agents/claim-extractor.md              sonnet, seed, series A, series B and series C
agents/annex-matcher.md                sonnet, seed, series A, series B and series C
agents/annex-classifier.md             sonnet, series A, series B and series C: which required document of the stage and model is each annex
agents/web-verifier.md                 sonnet, seed, series A, series B and series C, the only agent on the web
agents/contradiction-reviewer.md       opus, seed, series A, series B and series C
...
scripts/documents_gate.py              the stop decisions of the stages with a document list (series A, series B, series C): documents of the stage and model, contradictions
...
scripts/report.py                      final markdown, every stage, both languages, benchmarks, the series C plan vs actual table
...
- The tool can only stop on something false, never on something imprecise. At series A,
  series B and series C it also stops on a missing document, which is a fact about the file set, not about
  the company.
...
- The gap thresholds, the document minimums (24, 12, 36 months, 10 contracts at series A; 36,
  24, 12 months, 2 fiscal years, 20 contracts, 4 quarters at series B; 48, 36, 24 months, 3 fiscal years, 20 contracts, 8 quarters at series C) and the model weights are
...
- Several benchmarks have no dated source yet (sales cycle by segment, pipeline coverage, share
  ...
  above 50M USD ARR in the 2024-2025 reports). They are listed empty rather
  than invented.
