---
name: deck-reader
description: Use when the user runs /deck-reader with a path to a pitch deck PDF (and, at seed, series A and series B, the annexes the founder sent) and wants the deck-completeness report written next to it. One grid per stage (pre-seed, seed, series A, series B), one block per business model; the deck's announced stage picks the grid, the detected model picks the block. Never a verdict.
...
(`preseed`, `seed`, `series_a`, `series_b`); the grids are in `scripts/grids/`, the model blocks in
...
2. Read `announced_stage` in `WORK/profile.json` and set `GRID`:
   ...
   - anything else (`series-c-or-later`, `other`, `not_stated`): run
...
## 3A. Series A and series B: documents, first gate

The list is the one of the stage and the model block, see the grid and the model files (six
documents for SaaS at series A, ten at series B); a missing document stops the reading. No
coverage threshold. `GRID` is `series_a` or `series_b`, set in section 2.
...
## 4A. Series A and series B: claims and proof in the documents
...
4. Launch **one** `deck-reader:annex-matcher` agent with:
   ...
   There is no coverage gate at series A or series B: claims the documents do not cover are
   listed in the report as not covered and never stop the reading. No leftovers email: the
   document list replaces it. Continue at **5S** with `--grid GRID` in every command, then
   **6S** with `documents_gate.py contradictions --grid GRID` in place of
   `seed_gate.py contradictions`.

## 5S. Seed, series A and series B: web check
...
## 6S. Seed, series A and series B: double check, second gate

`GATE` = `seed_gate.py` at seed, `documents_gate.py` at series A and series B.
...
2. Launch the `deck-reader:block-checker` agents **in one message**, one per block, each with:
   ...
   `output_language=LANG`, and at seed, series A and series B `claims_path=CLAIMS`.
...
6. Seed, series A and series B: `python ${CLAUDE_PLUGIN_ROOT}/scripts/apply_proof_cap.py --grid GRID --profile WORK/profile.json --answers-dir P --claims CLAIMS`
...
2. Write the effective grid for the writer (model block applied, never the score):
   ...
   and at seed, series A and series B `claims_path=CLAIMS`.
...
3. Pre-seed: `python ${CLAUDE_PLUGIN_ROOT}/scripts/report.py --grid preseed --deck DECK --profile WORK/profile.json --answers-dir WORK/final --score WORK/score.json --reading WORK/reading.md --pages WORK/pages.json --lang LANG --out OUT`
   Seed, series A and series B: the same with `--grid GRID --annexes WORK/annexes.json --claims CLAIMS --gate WORK/gate.json --email EMAIL`.
   At seed, series A and series B the report shows, next to each figure, the benchmark of the
   model and the stage with its source and date. The script adds it; it never enters the score.

## 9. Finish

1. Unless `--keep-work` was given, delete `WORK`. Nothing persists outside `OUT`, its PDF twin
   and, at seed, series A and series B when something is missing, `EMAIL`.
2. Tell the user, in their language: the report paths (markdown and PDF); the stage, the grid
   ...
   at seed, series A and series B, how many claims were proven, confirmed, not covered,
   unverifiable, to probe, and whether an email draft was written and where; at series A and
   series B, which documents of the stage and model list were present. Nothing else. No opinion
