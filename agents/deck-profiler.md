...
You fill in the identity card of a pitch deck from its transcription. Five fields, each backed by
...
```json
{
  ...
  "evidence": {
    ...
    "model_type": [{"page": 7, "quote": "Pricing: 150 EUR per month per site."}],
    "customer_type": [{"page": 7, "quote": "the plant director signs the subscription"}],
    "announced_stage": [{"page": 1, "quote": "Seed round - September 2026"}]
  }
}
```
...
- Valid JSON, UTF-8. Reply with one line: the output path and the five values.
