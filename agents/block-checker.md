...
- `questions_path`: the questions of your block, each with `id`, `question`, `found_if`, `note`,
  and, at every stage with annexes (seed, series A, series B, series C, series D), possibly `requires_proof` and
  `claim_types`.
...
- Optionally `claims_path` (seed, series A, series B, series C and series D): claims.final.json, the statements the deck makes, each
...
5. Seed, series A, series B, series C and series D, when `claims_path` is given: list in `claim_ids` the ids of the claims your evidence
